        ...
        messages: [
            { role: 'system', content: characterPrompt },
            {
                ...
                content: [
                    ...
                    '卡牌 name 与 description 要体现这些经历或主题，不要写成和芙提雅无关的通用冰雪技能。description 中不要包含具体的数字、数值或规则说明，而是要写成与芙提雅相关的战术描述。',
                    '玩家输入的战斗风格是自由文本，只用于引导 flex 槽位的类别倾向和命名气质，不要套用固定模板，也不要把玩家要求改写成规则数值。',
                    '必须先理解 battleStyle 对牌型类别的真实要求，再决定 flex 槽位 category；如果玩家明确要求某些类别的牌更多，flex 槽位要优先满足这些类别。',
                    ...
                    '每个槽位会提供 rarity：blue 文案正常克制，purple 文案要显得更强大，gold 文案要显得无与伦比地强大；三种等级都必须继续参考芙提雅知识库经历，不要写成空泛强度形容。不要包含具体的数字和数值。',
                    '每个槽位会提供 effectScope：single 的语义描述必须写成与“单体/单目标/精准打击”效果相关；area 的语义描述必须写成与“群体/范围/多目标”效果相关。description 不得和 effectScope 矛盾，不要包含具体的数字和数值。',
