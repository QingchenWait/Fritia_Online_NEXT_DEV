...
- 战术考核打开期间 `#side-scroller-adventure` 会带 `is-side-combat-active`；`css/responsive.css` 只在该状态且视口 `min-width: 760px`、`max-height: 540px` 时启用低高度横屏压缩规则，缩小设定弹窗、HUD、手牌、侧边按钮、典藏牌库/规则/排行榜浮窗和卡池列表，并由 `side_scroller_adventure.js` 同步降低角色地面线与角色比例。该适配不作用于其他横板或 3D 功能。
...
15. 宽屏低高度视口（例如 1792x490、1280x390）下，未开始的“战术考核设定”弹窗、右侧战术文档/分数看板、左侧典藏牌库和战斗中的路线、生命、手牌、弃牌/刷新/卡池按钮都应缩放后保持可见，不互相压住。
