...
│   ├── mobile_common.css  # 移动端通用 UI（横屏/竖屏共享，不依赖 ml-active）
...
- 成功完成整条战术考核路线会增加好感度：标准 `+8`、困难 `+12`、传说 `+16`；失败、中途退出或单个事件完成不增加。
...
- 暖调闲聚普通 bot 访客（内置芬妮与玩家导入角色）加载或重新显示时会获得稳定 waypoint slot，并缓存一套从酒吧基础 waypoint 派生的专属偏移 waypoint，避免多个角色把同一个或非常相近的 waypoint 作为目标；该逻辑不做每帧检测，只改目标点分配，不做角色间避碰、停步、速度修正或位置推挤，固定琴诺 `special:cherno` 不参与。
...
12. 当前难度路线事件结束后显示路线完成，并按标准/困难/传说分别增加 8/12/16 好感度；芙提雅 HP 归零，或全局刷新次数为 0 且没有攻击/召唤牌、敌方流血、可用 `御驾亲征` 等伤害手段时，显示路线中断且不加好感。
...
15. 宽屏低高度视口（例如 1792x490、1280x390）下，未开始的“战术考核设定”弹窗、右侧战术文档/分数看板、左侧典藏牌库和战斗中的路线、生命、手牌、弃牌/刷新/卡池按钮都应随高度线性缩小后保持可见，不互相压住；极窄横屏下芙提雅生命 HUD 应居中显示在顶部路线信息下方，保留头像、隐藏 buff 状态，不遮挡敌方 `⚔️` 意图。
16. 退出小游戏后，旧房间南侧门按 `E` 进入暖调闲聚/显示入场券任务的原逻辑保持不变；暖调闲聚出口区域按 `E` 返回宿舍的原逻辑保持不变。
